...
class MainActivity : AppCompatActivity() {
    var peliculas = ArrayList<Contenido>()
    var series = ArrayList<Contenido>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val gridPeliculas: GridView = findViewById(R.id.gridPeliculas)
        val gridSeries: GridView = findViewById(R.id.gridSeries)

        cargarPeliculas()
        cargarSeries()

        val peliculasAdapter = ContenidoAdapter(this, peliculas)
        val seriesAdapter = ContenidoAdapter(this, series)
...
class ContenidoAdapter: BaseAdapter {
    var contenido = ArrayList<Contenido>()
    var context: Context? = null

    constructor(context: Context, contenido: ArrayList<Contenido>){
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        var contenidoItem = contenido[position]
        ...
        ivPelicula.setImageResource(contenidoItem.image)
        tituloPelicula.text = contenidoItem.titulo

        vista.setOnClickListener{
            var intent = Intent(context, DetalleContenido::class.java)
            intent.putExtra("titulo", contenidoItem.titulo)
            intent.putExtra("sinopsis", contenidoItem.sinopsis)
            intent.putExtra("header", contenidoItem.header)
            intent.putExtra("image", contenidoItem.image)

            // Aqui lo que hago es diferenciar el tipo de contenido
            if(contenidoItem is Pelicula){
                intent.putExtra("numberSeats", (20 - contenidoItem.seats.size))
            }
